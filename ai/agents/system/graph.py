...
Documenter - able to create code documentation. It can create files and write docs based on the code that has been written."""

code_dir = "/Users/jakubdulas/Documents/UPV/The-Real-AI-Software-Engineer/test"
project_board_dir = code_dir + "/.project/project_board.json"
docs_dir = code_dir + "/.project/docs"

pm = ProjectManager("gpt-4o-mini", project_board_dir, team)
coder = Coder(code_dir, "gpt-4o-mini")
documenter = Documenter("gpt-4o-mini", code_dir, docs_dir)
researcher = Researcher("gpt-4.1-nano")
...
def plan_project(state):
    ...
    return {
        "pm_messages": [("human", state.get("project_scope")), output["messages"][-1]],
        "project_board": output["project_board"],
    }


def start_sprint(state):
    print("Started new sprint")
...
def should_start_sprint(state):
    ...
    if len(sprints) <= current_sprint:
        print("should_start_sprint - going to the end")
        return "__end__"
    print("should_start_sprint - going to the start_sprint")
    return "start_sprint"


def complete_tasks(state):
    print("complete_tasks", state.get("project_board").keys())
    print(len(state.get("project_board")["sprints"]))
