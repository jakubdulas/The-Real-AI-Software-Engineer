...
main.py
-------
Provides the command-line interface for the note manager.
Presents a menu for creating, viewing, and deleting notes.
Handles user I/O, input validation, and hooks into the note model and storage API.
"""
import os
from note import Note
from storage import save_notes_to_file, load_notes_from_file
from utils import (
    is_empty_input, sanitize_string, format_timestamp,
    validate_menu_choice, validate_note_title, sanitize_text
)

def main():
    """
    Run the main loop for the command-line note manager application.

    Presents a menu to the user for creating, viewing, and deleting notes,
    handles all input validation and persistence, and exits when prompted.
    """
    FILENAME = 'notes.json'
        ...
        if choice == '1':
            while True:
                ...
                if not validate_note_title(title):
                    print("Error: Title must be 1-100 characters and not just whitespace.")
                else:
